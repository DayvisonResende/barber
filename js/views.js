Object.assign(App, {
    ...
    renderShopManagement() {
        ...
        if (tab === 'barbers') {
            contentHtml = `
                <div class="space-y-4 fade-in">
                    ...
                    <div class="card-bg border border-theme rounded-2xl p-5 shadow-sm space-y-4">
                        <h3 class="text-sm font-bold text-theme flex items-center gap-2"><i data-lucide="user-plus" class="w-4 h-4 text-emerald-500"></i> Cadastrar Profissional</h3>
                        
                        <div class="space-y-2">
                            <label class="text-[10px] text-muted-theme uppercase font-bold tracking-wider">Nome do Profissional *</label>
                            <input type="text" id="new-barber-name" placeholder="Ex: Carlos Silva" class="w-full input-bg border border-zinc-700 rounded-xl p-3 text-theme focus:border-amber-500 outline-none transition-colors" />
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div class="space-y-2">
                                <label class="text-[10px] text-muted-theme uppercase font-bold tracking-wider">Vincular Conta (UUID do Auth)</label>
                                <input type="text" id="new-barber-userid" placeholder="Opcional. Ex: a1b2c3d4-..." class="w-full input-bg border border-zinc-700 rounded-xl p-3 text-theme focus:border-amber-500 outline-none transition-colors text-xs" />
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] text-muted-theme uppercase font-bold tracking-wider">Foto de Perfil (URL)</label>
                                <input type="text" id="new-barber-avatar" placeholder="Opcional. Deixe vazio para foto padrão." class="w-full input-bg border border-zinc-700 rounded-xl p-3 text-theme focus:border-amber-500 outline-none transition-colors text-xs" />
                            </div>
                        </div>
                        <button onclick="App.addBarber()" class="w-full py-3 rounded-xl font-bold transition-all duration-200 bg-emerald-500 text-zinc-950 hover:bg-emerald-400 shadow-lg shadow-emerald-500/20 active:scale-95 flex items-center justify-center gap-2">
                            <i data-lucide="plus" class="w-4 h-4"></i> Adicionar Profissional
                        </button>
                    </div>
            ...
            contentHtml = `
                <div class="space-y-4 fade-in">
                    <div class="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 mb-2 flex items-start gap-3">
                        <i data-lucide="info" class="w-5 h-5 text-amber-500 mt-0.5"></i>
                        <p class="text-xs text-amber-200/80 font-medium leading-relaxed">
                            Ao bloquear um horário aqui, ele ficará indisponível no calendário da barbearia para <b>todos os profissionais</b> (Ex: Horário de Almoço).
                        </p>
                    </div>
                    
                    <div class="grid grid-cols-4 gap-2">
                        ${AVAILABLE_TIMES.map(time => {
                            const isBlocked = this.state.blockedTimes.includes(time);
                            return `
                                <button onclick="App.toggleTimeBlock('${time}')" class="py-3 rounded-xl border flex items-center justify-center font-bold text-sm transition-all duration-200 active:scale-95 ${isBlocked ? 'bg-red-500/20 border-red-500/50 text-red-500' : 'card-bg border-theme text-theme hover:border-amber-500'}">
                                    ${time} ${isBlocked ? '<i data-lucide="lock" class="w-3 h-3 ml-1"></i>' : ''}
                                </button>
                            `;
                        }).join('')}
                    </div>
        ...
        return `
            <div class="space-y-6 fade-in slide-in-up pb-6">
                ...
                <div class="flex items-center justify-between mb-4 border-b border-theme/50 pb-4">
                    <div>
                        <h2 class="text-2xl font-bold text-theme flex items-center gap-2"><i data-lucide="shield-check" class="w-6 h-6 text-amber-500"></i> Painel Admin</h2>
                        <p class="text-xs text-muted-theme mt-1">Gestão de sistema e profissionais.</p>
                    </div>
                ...
                <div class="flex gap-2 p-1 bg-zinc-900 rounded-xl overflow-x-auto scrollbar-hide border border-zinc-800">
                    <button onclick="App.setAdminShopTab('barbers')" class="flex-1 min-w-[90px] text-[11px] font-bold py-2.5 rounded-lg transition-colors flex items-center justify-center gap-1.5 ${tab === 'barbers' ? 'bg-zinc-800 text-amber-500 shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}">
                        <i data-lucide="users" class="w-3.5 h-3.5"></i> Profissionais
                    </button>
