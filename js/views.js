Object.assign(App, {
    ...
    renderRelatorios() {
        ...
        let filteredTxs = this.state.completedTransactions.filter(tx => {
            ...
            if (this.state.paymentMethodFilter !== 'all') {
                return tx.paymentMethod === this.state.paymentMethodFilter;
            }
                                ...
                                <div class="card-bg rounded-xl border border-theme p-3 shadow-sm flex items-center justify-between hover:border-zinc-700 transition-colors">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[10px] text-muted-theme flex items-center gap-1.5 mt-0.5">
                                                <span class="bg-zinc-800 px-1.5 py-0.5 rounded border border-zinc-700 font-bold text-amber-500 uppercase tracking-tighter">${tx.paymentMethod}</span>
                                                <span>•</span>
                                                <span class="truncate">${tx.service.name}</span>
                                                ${isManagement && tx.barberName ? `
                                                    <span>•</span>
                                                    <span class="text-amber-500 font-bold uppercase tracking-tighter bg-amber-500/10 px-1 rounded">${tx.barberName.split(' ')[0]}</span>
                                                ` : ''}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <p class="font-bold text-theme">R$ ${tx.numericValue.toFixed(2).replace('.', ',')}</p>
