                                    ...
                                    return `
                                        <div onclick="App.toggleService(${svc.id})" class="p-4 rounded-xl border cursor-pointer transition-all ${isSelected ? 'border-amber-500 bg-amber-500/10' : 'border-theme input-bg hover:border-zinc-700'} flex items-center gap-3">
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <p class="text-[10px] text-muted-theme mt-0.5 uppercase tracking-tighter">${svc.duration} • R$ ${svc.priceValue.toFixed(2).replace('.', ',')}</p>
                                            </div>
                                        </div>
                                    `;
        ...
        if (['admin', 'manager', 'barber'].includes(this.state.role)) {
            ...
            return `
                <div class="space-y-6 fade-in slide-in-up">
                                            ...
                                            <div class="flex flex-wrap gap-1.5 mt-1.5">
                                                ...
                                                ` : `
                                                    <button onclick="App.initEditPrice('${apt.id}')" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-500 text-[10px] font-bold uppercase tracking-wider border border-emerald-500/20 hover:bg-emerald-500/20 transition-all" title="Editar Valor">
                                                        <i data-lucide="dollar-sign" class="w-3 h-3"></i> ${apt.service.price}
                                                    </button>
                                                `}
                    ...
                    `}
                    ${this.state.showingSplitPaymentId ? this.renderSplitPaymentModal(this.state.showingSplitPaymentId) : ''}
                </div>
                    ...
                    <div class="space-y-3">
                        ${SERVICES.map(s => `
                            <div class="card-bg border border-theme rounded-2xl p-4 shadow-sm flex items-center justify-between transition-all group hover:border-amber-500">
                                <div>
                                    <h4 class="font-bold text-theme">${App.escapeHTML(s.name)}</h4>
    ...
    renderSplitPaymentModal(id) {
        ...
        return `
            <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm fade-in">
                <div class="card-bg w-full max-w-sm rounded-[2.5rem] p-8 border border-theme shadow-2xl scale-in relative overflow-hidden">
                    <!-- Detalhe de fundo -->
                    ...
                    <div class="relative">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <button onclick="App.state.showingSplitPaymentId = null; App.render();" class="w-10 h-10 rounded-full bg-zinc-900 flex items-center justify-center text-zinc-500 hover:text-white transition-colors border border-zinc-800">
                                <i data-lucide="x" class="w-5 h-5"></i>
                            </button>
                        </div>

                        <div class="bg-zinc-900/50 rounded-2xl p-4 border border-theme mb-6 flex justify-between items-center">
                            <div>
            ...
            <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm fade-in">
                <div class="card-bg w-full max-w-sm rounded-[2.5rem] p-8 border border-theme shadow-2xl scale-in relative overflow-hidden flex flex-col max-h-[80vh]">
                    ...
                    <div class="relative flex flex-col h-full">
                        ...
                        <div class="overflow-y-auto pr-2 space-y-2 mb-6 flex-1 custom-scrollbar">
                            ${SERVICES.map(svc => {
