Object.assign(App, {
    ...
    render() {
        ...
        // Re-aplicar máscaras e recriar os ícones Lucide em todo o app
        if (window.lucide) {
            lucide.createIcons();
            this.applyMasks();
        }
        ...
        if (modalContainer) {
            if (this.state.showingSplitPaymentId) {
                // Modal de pagamento dividido: renderizado FORA do elemento com transform
                // para que position:fixed funcione em relação ao viewport real.
                modalContainer.innerHTML = this.renderSplitPaymentModal(this.state.showingSplitPaymentId);
                if (window.lucide) lucide.createIcons({ root: modalContainer });
            } else if (this.state.showRegistrationSuccess) {
    ...
    filterGestaoClients(term) {
        if (!term) term = '';
        term = term.toLowerCase().trim();
        document.querySelectorAll('.client-gestao-card').forEach(card => {
            const searchStr = card.dataset.search ? card.dataset.search.toLowerCase() : '';
