Object.assign(App, {
    async init() {
        ...
        // Verificar sessão e carregar dados em paralelo
    ...
    async loadInitialData() {
        ...
        const [svcsRes, brbsRes, blocksRes, settingsRes, aptsRes] = await Promise.all([
        ...
        if (aptsRes.data) this.state.allAppointmentsForStats = aptsRes.data;
        ...
        supabaseClient.channel('agenda-realtime')
            .on(
                ...
                async (payload) => {
                    ...
                    // Se for um novo agendamento, mostra notificação visual para o barbeiro
                    if (payload.eventType === 'INSERT' && (this.state.role === 'barber' || this.state.role === 'admin')) {
    ...
    async saveShopSettings() {
        ...
        const newSettings = {
            ...
            commission_rate: document.getElementById('shop-commission') ? parseInt(document.getElementById('shop-commission').value) : this.state.shopSettings.commission_rate
        };
    ...
    async updateCommissionRate(newRate) {
        ...
        if (!error) {
            ...
            this.showNotification('Sucesso', `Repasse atualizado para ${rate}%`);
            this.render();
        } else {
