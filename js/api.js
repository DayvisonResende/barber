        ...
        supabaseClient.channel('agenda-realtime')
            ...
            .on(
                ...
                async (payload) => {
                    ...
                    // Se for um novo agendamento, mostra notificação visual para o barbeiro/admin
                    if (payload.eventType === 'INSERT' && (this.state.role === 'barber' || this.state.role === 'admin' || this.state.role === 'manager')) {
                        const newApt = payload.new;
                        this.showNotification('Novo Agendamento!', `Um novo horário foi marcado para ${newApt.time}.`);
                        // Feedback háptico (vibração no mobile se disponível)
                        this.haptic('success');
                    }
            ...
            .on(
                ...
                async (payload) => {
                    console.log('⚡ Atualização Instantânea (Financeiro)!', payload);
                    if (this.state.isAuthenticated) {
                        // IMPORTANTE: Recarregar também os agendamentos aqui.
                        // Quando o barbeiro finaliza um atendimento, ele insere uma transação E
                        // atualiza o status do agendamento para 'completed'. O Supabase Realtime
                        // com RLS suprime o evento UPDATE do agendamento para o cliente (pois o
                        // row 'completed' não passa mais pela policy do cliente que filtra 'pending').
                        // Ao carregar aqui, garantimos que o cliente vê a remoção em tempo real.
                        await Promise.all([
                            this.loadTransactions(),
                            this.loadAppointments()
                        ]);
            ...
            // --- EQUIPE E SERVIÇOS (CATÁLOGO) ---
            .on(
                'postgres_changes', 
                { event: '*', schema: 'public', table: 'barbers' }, 
                async () => {
                    console.log('⚡ Atualização Instantânea (Equipe)!');
                    await this.loadInitialData();
                    this.render();
                }
            )
            .on(
                ...
                async () => {
                    console.log('⚡ Atualização Instantânea (Serviços)!');
                    await this.loadInitialData();
                    this.render();
                }
            )
            ...
            .on(
                ...
                async () => {
                    console.log('⚡ Atualização Instantânea (Configurações)!');
                    await this.loadInitialData();
                    this.render();
                }
            )
