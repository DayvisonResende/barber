...
// We will find the start string, then the `}).join('')}` string that follows it, and extract up to the closing backtick.
const startStr = "const clientInitial = (apt.clientName[0] || 'C').toUpperCase();";
const startIdx = content.indexOf(startStr);
if (startIdx === -1) {
    console.error("Could not find start bounds!");
    process.exit(1);
}

const mapEndStr = "            }).join('')}";
const mapEndIdx = content.indexOf(mapEndStr, startIdx);
if (mapEndIdx === -1) {
    console.error("Could not find map end bounds!");
    process.exit(1);
}

// Extract the block that is inside the map body
let actualStart = content.lastIndexOf('\n', startIdx) + 1;
let actualEnd = content.lastIndexOf('\n', mapEndIdx); // the line before the mapEndStr

let cardBlock = content.substring(actualStart, actualEnd);
...
const newMapEndIdx = content.indexOf(mapEndStr, newActualStart);
const newActualEnd = content.lastIndexOf('\n', newMapEndIdx);

const blockToReplace = content.substring(newActualStart, newActualEnd);
// Keep the exact same indentation that the original block had!
...
// 6. Add list toggle button (safe replacement)
const listHeaderStr1 = `                            <h2 class="text-2xl font-bold text-theme">Agenda</h2>\r\n                            <span class="bg-amber-500/10 text-amber-500 px-3 py-1 rounded-full text-sm font-medium">\r\n                                \${filteredApts.length} serviço(s)\r\n                            </span>`;
const listHeaderStr2 = `                            <h2 class="text-2xl font-bold text-theme">Agenda</h2>\n                            <span class="bg-amber-500/10 text-amber-500 px-3 py-1 rounded-full text-sm font-medium">\n                                \${filteredApts.length} serviço(s)\n                            </span>`;
                        
const newHeaderStr = `                            <h2 class="text-2xl font-bold text-theme">Agenda</h2>
                            <div class="flex items-center gap-2">
                                <span class="bg-amber-500/10 text-amber-500 px-3 py-1 rounded-full text-sm font-medium">
                                    \${filteredApts.length} serviço(s)
                                </span>
...
if(content.includes(listHeaderStr1)) content = content.replace(listHeaderStr1, newHeaderStr);
else content = content.replace(listHeaderStr2, newHeaderStr);
